...
const CoverUndangan = ({ opened, bukaUndangan, guest }) => {
    useEffect(() => {
        AOS.init({
            duration: 500,
            once: false,
    ...
    return (
        <section id="cover" className={`${styles.cover} d-flex align-items-center`}>
            <div className="container">
                ...
                <div className={styles.the_wedding_of} data-aos="fade-up">
                    The Wedding Of
                </div>
                <div className={styles.couple_name} data-aos="zoom-in-up">
                    Mega & Erik
                </div>
                <div className={styles.nikah_date} data-aos="fade-up">
                    17 . 07 . 2022
                </div>
                <div className="text-center mt-4" data-aos="flip-up" data-aos-delay="500">
                    <div>Kepada Yth Bapak/Ibu/Saudara/i:</div>
                    <div className={styles.invited_name}>{guest.name}</div>
                    <div className={styles.invited_info}>{guest.address}</div>
                    <div className="d-flex justify-content-center mt-4">
                        {!opened && (
                            <button className={`btn ${styles.btn_open}`} onClick={() => bukaUndangan()}>
                                Buka Undangan
                            </button>
                        )}
                    </div>
